...
for(int i=0;i<n;++i)
{
  printf("Enter arrival time, burst time and priority of 
process %d : ",i+1);
  p[i].pno = i+1;
  scanf("%d",&p[i].at);
  scanf("%d",&p[i].bt);
...
printf("Pno  Arrival time  Burst time  Waiting time  Turnaround 
time  Priority\n");
for(int i=0;i<n;++i)
{
  printf("%3d  %11d  %9d  %12d  %14d  %8d\n",p[i].pno,p[i].at,
p[i].bt,p[i].wt,p[i].tt,p[i].p);
}
avg_wt=0;
avg_tt=0;
for(int i=0;i<n;++i)
...
for(i=1;i<n;++i)
{
  t=p1[i-1].at+p1[i-1].wt+p1[i-1].bt;
  for(j=i;p1[j].at<=t&&j<n;++j);
  for(k=i;k<j;++k)
    for(m=i;m<j-k-1+i;++m)
      if(p1[m+1].p<p1[m].p)
        swap(&p1[m+1],&p1[m]);
